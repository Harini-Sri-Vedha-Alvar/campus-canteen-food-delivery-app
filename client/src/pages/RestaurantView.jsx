  ...
  >
    ...
    /* Menu Section */
      ...
      <div className="container mx-auto px-4 mt-12 max-w-5xl">
        ...
        {isLoadingMenu ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => <div key={i} className="h-32 bg-muted rounded-2xl animate-pulse" />)}
          </div> : menuItems.length === 0 ? <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-border">
            ...
            {menuItems.map((item, i) => <MenuItemCard key={item.id} item={item} index={i} />)}
          </div>}
      </div>
    </div>;
}
export {
  RestaurantView as default
};
