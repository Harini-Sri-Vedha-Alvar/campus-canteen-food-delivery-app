...
import { Store, ListPlus, ReceiptText, Home, LogOut } from "lucide-react";
      ...
      <aside className="w-full md:w-64 bg-card border-b md:border-b-0 md:border-r border-border p-6 flex flex-col h-auto md:h-screen sticky top-0">
        ...
        <nav className="flex-1 space-y-2 flex flex-row md:flex-col overflow-x-auto md:overflow-visible pb-2 md:pb-0">
          {links.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all whitespace-nowrap md:whitespace-normal ${isActive ? 'bg-accent text-white shadow-bold' : 'text-muted-foreground hover:bg-accent/10 hover:text-accent'}`}>
