...
function AdminOrders() {
  ...
  return <AdminLayout>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y border-border">
                {orders.map((order) => <tr key={order.id} className="hover:bg-muted/10 transition-colors">
    ...
    className={`font-bold text-sm rounded-xl px-3 py-2 border-2 outline-none cursor-pointer appearance-none
                          ${order.status === "pending" ? "bg-orange-100 text-orange-700 border-orange-200" : order.status === "delivered" ? "bg-green-100 text-green-700 border-green-200" : "bg-blue-100 text-blue-700 border-blue-200"}`}
  >
