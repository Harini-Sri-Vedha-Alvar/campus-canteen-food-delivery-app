/**
 * Generated by orval v8.6.2 🍺
 * Do not edit manually.
 * Api
 * FoodRush - Food Delivery App API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from "zod";

/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  status: zod.string(),
});

/**
 * @summary Register a new user
 */
export const RegisterUserBody = zod.object({
  name: zod.string(),
  email: zod.string(),
  password: zod.string(),
  phone: zod.string().optional(),
  address: zod.string().optional(),
});

/**
 * @summary Login
 */
export const LoginUserBody = zod.object({
  email: zod.string(),
  password: zod.string(),
});

export const LoginUserResponse = zod.object({
  user: zod.object({
    id: zod.string(),
    name: zod.string(),
    email: zod.string(),
    phone: zod.string().nullish(),
    address: zod.string().nullish(),
    role: zod.string(),
    coins: zod.number(),
    avatar: zod.string().nullish(),
    createdAt: zod.string(),
  }),
  token: zod.string(),
});

/**
 * @summary Get current user
 */
export const GetCurrentUserResponse = zod.object({
  id: zod.string(),
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().nullish(),
  address: zod.string().nullish(),
  role: zod.string(),
  coins: zod.number(),
  avatar: zod.string().nullish(),
  createdAt: zod.string(),
});

/**
 * @summary List all restaurants
 */
export const ListRestaurantsQueryParams = zod.object({
  cuisine: zod.coerce.string().optional(),
  search: zod.coerce.string().optional(),
  rating: zod.coerce.number().optional(),
  sort: zod.coerce.string().optional(),
});

export const ListRestaurantsResponseItem = zod.object({
  id: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  cuisine: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  deliveryTime: zod.string(),
  deliveryFee: zod.number(),
  minOrder: zod.number(),
  address: zod.string(),
  isOpen: zod.boolean(),
  featured: zod.boolean(),
  tags: zod.array(zod.string()),
  menu: zod
    .array(
      zod.object({
        id: zod.string(),
        restaurantId: zod.string(),
        restaurantName: zod.string(),
        name: zod.string(),
        description: zod.string().nullish(),
        price: zod.number(),
        category: zod.string(),
        image: zod.string().nullish(),
        rating: zod.number(),
        totalRatings: zod.number(),
        isVeg: zod.boolean(),
        isAvailable: zod.boolean(),
        isBestseller: zod.boolean(),
        tags: zod.array(zod.string()),
        suggestedWith: zod.array(zod.string()),
      }),
    )
    .optional(),
  createdAt: zod.string(),
});
export const ListRestaurantsResponse = zod.array(ListRestaurantsResponseItem);

/**
 * @summary Get restaurant by ID
 */
export const GetRestaurantParams = zod.object({
  id: zod.coerce.string(),
});

export const GetRestaurantResponse = zod.object({
  id: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  cuisine: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  deliveryTime: zod.string(),
  deliveryFee: zod.number(),
  minOrder: zod.number(),
  address: zod.string(),
  isOpen: zod.boolean(),
  featured: zod.boolean(),
  tags: zod.array(zod.string()),
  menu: zod
    .array(
      zod.object({
        id: zod.string(),
        restaurantId: zod.string(),
        restaurantName: zod.string(),
        name: zod.string(),
        description: zod.string().nullish(),
        price: zod.number(),
        category: zod.string(),
        image: zod.string().nullish(),
        rating: zod.number(),
        totalRatings: zod.number(),
        isVeg: zod.boolean(),
        isAvailable: zod.boolean(),
        isBestseller: zod.boolean(),
        tags: zod.array(zod.string()),
        suggestedWith: zod.array(zod.string()),
      }),
    )
    .optional(),
  createdAt: zod.string(),
});

/**
 * @summary Get menu for a restaurant
 */
export const GetRestaurantMenuParams = zod.object({
  id: zod.coerce.string(),
});

export const GetRestaurantMenuQueryParams = zod.object({
  category: zod.coerce.string().optional(),
});

export const GetRestaurantMenuResponseItem = zod.object({
  id: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  isVeg: zod.boolean(),
  isAvailable: zod.boolean(),
  isBestseller: zod.boolean(),
  tags: zod.array(zod.string()),
  suggestedWith: zod.array(zod.string()),
});
export const GetRestaurantMenuResponse = zod.array(
  GetRestaurantMenuResponseItem,
);

/**
 * @summary Update menu item (admin)
 */
export const UpdateMenuItemParams = zod.object({
  id: zod.coerce.string(),
});

export const UpdateMenuItemBody = zod.object({
  restaurantId: zod.string(),
  name: zod.string(),
  description: zod.string().optional(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().optional(),
  isVeg: zod.boolean().optional(),
  isAvailable: zod.boolean().optional(),
  isBestseller: zod.boolean().optional(),
  tags: zod.array(zod.string()).optional(),
  suggestedWith: zod.array(zod.string()).optional(),
});

export const UpdateMenuItemResponse = zod.object({
  id: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  isVeg: zod.boolean(),
  isAvailable: zod.boolean(),
  isBestseller: zod.boolean(),
  tags: zod.array(zod.string()),
  suggestedWith: zod.array(zod.string()),
});

/**
 * @summary List all menu items (admin)
 */
export const ListMenuItemsResponseItem = zod.object({
  id: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  isVeg: zod.boolean(),
  isAvailable: zod.boolean(),
  isBestseller: zod.boolean(),
  tags: zod.array(zod.string()),
  suggestedWith: zod.array(zod.string()),
});
export const ListMenuItemsResponse = zod.array(ListMenuItemsResponseItem);

/**
 * @summary Create menu item (admin)
 */
export const CreateMenuItemBody = zod.object({
  restaurantId: zod.string(),
  name: zod.string(),
  description: zod.string().optional(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().optional(),
  isVeg: zod.boolean().optional(),
  isAvailable: zod.boolean().optional(),
  isBestseller: zod.boolean().optional(),
  tags: zod.array(zod.string()).optional(),
  suggestedWith: zod.array(zod.string()).optional(),
});

/**
 * @summary Delete menu item (admin)
 */
export const DeleteMenuItemParams = zod.object({
  id: zod.coerce.string(),
});

/**
 * @summary Get user cart
 */
export const GetCartResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  restaurantId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      id: zod.string(),
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      restaurantId: zod.string(),
    }),
  ),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  discount: zod.number(),
});

/**
 * @summary Add item to cart
 */
export const AddToCartBody = zod.object({
  menuItemId: zod.string(),
  quantity: zod.number(),
});

export const AddToCartResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  restaurantId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      id: zod.string(),
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      restaurantId: zod.string(),
    }),
  ),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  discount: zod.number(),
});

/**
 * @summary Update cart item quantity
 */
export const UpdateCartItemParams = zod.object({
  itemId: zod.coerce.string(),
});

export const UpdateCartItemBody = zod.object({
  quantity: zod.number(),
});

export const UpdateCartItemResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  restaurantId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      id: zod.string(),
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      restaurantId: zod.string(),
    }),
  ),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  discount: zod.number(),
});

/**
 * @summary Remove item from cart
 */
export const RemoveCartItemParams = zod.object({
  itemId: zod.coerce.string(),
});

export const RemoveCartItemResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  restaurantId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      id: zod.string(),
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      restaurantId: zod.string(),
    }),
  ),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  discount: zod.number(),
});

/**
 * @summary List user orders
 */
export const listOrdersResponseItemsItemRatingMax = 5;

export const listOrdersResponseRestaurantRatingMax = 5;

export const ListOrdersResponseItem = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      rating: zod
        .number()
        .min(1)
        .max(listOrdersResponseItemsItemRatingMax)
        .nullish(),
      review: zod.string().nullish(),
    }),
  ),
  status: zod.string(),
  paymentMethod: zod.string(),
  paymentStatus: zod.string(),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  discount: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  coinsUsed: zod.number(),
  coinsEarned: zod.number(),
  deliveryAddress: zod.string(),
  estimatedDeliveryTime: zod.string().nullish(),
  trackingSteps: zod.array(
    zod.object({
      step: zod.string(),
      label: zod.string(),
      completedAt: zod.string().nullish(),
      isCompleted: zod.boolean(),
    }),
  ),
  createdAt: zod.string(),
  updatedAt: zod.string(),
  restaurantRating: zod
    .number()
    .min(1)
    .max(listOrdersResponseRestaurantRatingMax)
    .nullish(),
  restaurantReview: zod.string().nullish(),
});
export const ListOrdersResponse = zod.array(ListOrdersResponseItem);

/**
 * @summary Place an order
 */
export const CreateOrderBody = zod.object({
  restaurantId: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      quantity: zod.number(),
    }),
  ),
  paymentMethod: zod.string(),
  deliveryAddress: zod.string(),
  couponCode: zod.string().optional(),
  useCoins: zod.boolean().optional(),
});

/**
 * @summary Get order by ID
 */
export const GetOrderParams = zod.object({
  id: zod.coerce.string(),
});

export const getOrderResponseItemsItemRatingMax = 5;

export const getOrderResponseRestaurantRatingMax = 5;

export const GetOrderResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      rating: zod
        .number()
        .min(1)
        .max(getOrderResponseItemsItemRatingMax)
        .nullish(),
      review: zod.string().nullish(),
    }),
  ),
  status: zod.string(),
  paymentMethod: zod.string(),
  paymentStatus: zod.string(),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  discount: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  coinsUsed: zod.number(),
  coinsEarned: zod.number(),
  deliveryAddress: zod.string(),
  estimatedDeliveryTime: zod.string().nullish(),
  trackingSteps: zod.array(
    zod.object({
      step: zod.string(),
      label: zod.string(),
      completedAt: zod.string().nullish(),
      isCompleted: zod.boolean(),
    }),
  ),
  createdAt: zod.string(),
  updatedAt: zod.string(),
  restaurantRating: zod
    .number()
    .min(1)
    .max(getOrderResponseRestaurantRatingMax)
    .nullish(),
  restaurantReview: zod.string().nullish(),
});

/**
 * @summary Update order status (admin)
 */
export const UpdateOrderStatusParams = zod.object({
  id: zod.coerce.string(),
});

export const UpdateOrderStatusBody = zod.object({
  status: zod.string(),
  estimatedDeliveryTime: zod.string().optional(),
});

export const updateOrderStatusResponseItemsItemRatingMax = 5;

export const updateOrderStatusResponseRestaurantRatingMax = 5;

export const UpdateOrderStatusResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      rating: zod
        .number()
        .min(1)
        .max(updateOrderStatusResponseItemsItemRatingMax)
        .nullish(),
      review: zod.string().nullish(),
    }),
  ),
  status: zod.string(),
  paymentMethod: zod.string(),
  paymentStatus: zod.string(),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  discount: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  coinsUsed: zod.number(),
  coinsEarned: zod.number(),
  deliveryAddress: zod.string(),
  estimatedDeliveryTime: zod.string().nullish(),
  trackingSteps: zod.array(
    zod.object({
      step: zod.string(),
      label: zod.string(),
      completedAt: zod.string().nullish(),
      isCompleted: zod.boolean(),
    }),
  ),
  createdAt: zod.string(),
  updatedAt: zod.string(),
  restaurantRating: zod
    .number()
    .min(1)
    .max(updateOrderStatusResponseRestaurantRatingMax)
    .nullish(),
  restaurantReview: zod.string().nullish(),
});

/**
 * @summary Rate order items and restaurant
 */
export const RateOrderParams = zod.object({
  id: zod.coerce.string(),
});

export const rateOrderBodyRestaurantRatingMax = 5;

export const rateOrderBodyItemRatingsItemRatingMax = 5;

export const RateOrderBody = zod.object({
  restaurantRating: zod
    .number()
    .min(1)
    .max(rateOrderBodyRestaurantRatingMax)
    .optional(),
  restaurantReview: zod.string().optional(),
  itemRatings: zod
    .array(
      zod.object({
        menuItemId: zod.string().optional(),
        rating: zod
          .number()
          .min(1)
          .max(rateOrderBodyItemRatingsItemRatingMax)
          .optional(),
        review: zod.string().optional(),
      }),
    )
    .optional(),
});

export const rateOrderResponseItemsItemRatingMax = 5;

export const rateOrderResponseRestaurantRatingMax = 5;

export const RateOrderResponse = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      rating: zod
        .number()
        .min(1)
        .max(rateOrderResponseItemsItemRatingMax)
        .nullish(),
      review: zod.string().nullish(),
    }),
  ),
  status: zod.string(),
  paymentMethod: zod.string(),
  paymentStatus: zod.string(),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  discount: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  coinsUsed: zod.number(),
  coinsEarned: zod.number(),
  deliveryAddress: zod.string(),
  estimatedDeliveryTime: zod.string().nullish(),
  trackingSteps: zod.array(
    zod.object({
      step: zod.string(),
      label: zod.string(),
      completedAt: zod.string().nullish(),
      isCompleted: zod.boolean(),
    }),
  ),
  createdAt: zod.string(),
  updatedAt: zod.string(),
  restaurantRating: zod
    .number()
    .min(1)
    .max(rateOrderResponseRestaurantRatingMax)
    .nullish(),
  restaurantReview: zod.string().nullish(),
});

/**
 * @summary List reviews
 */
export const ListReviewsQueryParams = zod.object({
  menuItemId: zod.coerce.string().optional(),
  restaurantId: zod.coerce.string().optional(),
});

export const ListReviewsResponseItem = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  userAvatar: zod.string().nullish(),
  menuItemId: zod.string().nullish(),
  restaurantId: zod.string().nullish(),
  orderId: zod.string(),
  rating: zod.number(),
  comment: zod.string(),
  createdAt: zod.string(),
});
export const ListReviewsResponse = zod.array(ListReviewsResponseItem);

/**
 * @summary Create a review
 */
export const CreateReviewBody = zod.object({
  menuItemId: zod.string().optional(),
  restaurantId: zod.string().optional(),
  orderId: zod.string(),
  rating: zod.number(),
  comment: zod.string(),
});

/**
 * @summary Validate a coupon code
 */
export const ValidateCouponBody = zod.object({
  code: zod.string(),
  orderAmount: zod.number(),
});

export const ValidateCouponResponse = zod.object({
  id: zod.string(),
  code: zod.string(),
  description: zod.string(),
  discountType: zod.string(),
  discountValue: zod.number(),
  minOrderAmount: zod.number(),
  maxDiscount: zod.number().nullish(),
  isActive: zod.boolean(),
  expiresAt: zod.string().nullish(),
  usageLimit: zod.number().nullish(),
  usedCount: zod.number(),
});

/**
 * @summary List all available coupons
 */
export const ListCouponsResponseItem = zod.object({
  id: zod.string(),
  code: zod.string(),
  description: zod.string(),
  discountType: zod.string(),
  discountValue: zod.number(),
  minOrderAmount: zod.number(),
  maxDiscount: zod.number().nullish(),
  isActive: zod.boolean(),
  expiresAt: zod.string().nullish(),
  usageLimit: zod.number().nullish(),
  usedCount: zod.number(),
});
export const ListCouponsResponse = zod.array(ListCouponsResponseItem);

/**
 * @summary Create coupon (admin)
 */
export const CreateCouponBody = zod.object({
  code: zod.string(),
  description: zod.string(),
  discountType: zod.string(),
  discountValue: zod.number(),
  minOrderAmount: zod.number(),
  maxDiscount: zod.number().optional(),
  usageLimit: zod.number().optional(),
  expiresAt: zod.string().optional(),
});

/**
 * @summary Get user coins balance
 */
export const GetUserCoinsResponse = zod.object({
  coins: zod.number(),
  equivalentValue: zod.number(),
});

/**
 * @summary Update user profile
 */
export const UpdateUserProfileBody = zod.object({
  name: zod.string().optional(),
  phone: zod.string().optional(),
  address: zod.string().optional(),
  avatar: zod.string().optional(),
});

export const UpdateUserProfileResponse = zod.object({
  id: zod.string(),
  name: zod.string(),
  email: zod.string(),
  phone: zod.string().nullish(),
  address: zod.string().nullish(),
  role: zod.string(),
  coins: zod.number(),
  avatar: zod.string().nullish(),
  createdAt: zod.string(),
});

/**
 * @summary Create restaurant (admin)
 */
export const CreateRestaurantBody = zod.object({
  name: zod.string(),
  description: zod.string().optional(),
  cuisine: zod.string(),
  image: zod.string().optional(),
  deliveryTime: zod.string(),
  deliveryFee: zod.number(),
  minOrder: zod.number(),
  address: zod.string(),
  isOpen: zod.boolean().optional(),
  featured: zod.boolean().optional(),
  tags: zod.array(zod.string()).optional(),
});

/**
 * @summary Update restaurant (admin)
 */
export const UpdateRestaurantParams = zod.object({
  id: zod.coerce.string(),
});

export const UpdateRestaurantBody = zod.object({
  name: zod.string(),
  description: zod.string().optional(),
  cuisine: zod.string(),
  image: zod.string().optional(),
  deliveryTime: zod.string(),
  deliveryFee: zod.number(),
  minOrder: zod.number(),
  address: zod.string(),
  isOpen: zod.boolean().optional(),
  featured: zod.boolean().optional(),
  tags: zod.array(zod.string()).optional(),
});

export const UpdateRestaurantResponse = zod.object({
  id: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  cuisine: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  deliveryTime: zod.string(),
  deliveryFee: zod.number(),
  minOrder: zod.number(),
  address: zod.string(),
  isOpen: zod.boolean(),
  featured: zod.boolean(),
  tags: zod.array(zod.string()),
  menu: zod
    .array(
      zod.object({
        id: zod.string(),
        restaurantId: zod.string(),
        restaurantName: zod.string(),
        name: zod.string(),
        description: zod.string().nullish(),
        price: zod.number(),
        category: zod.string(),
        image: zod.string().nullish(),
        rating: zod.number(),
        totalRatings: zod.number(),
        isVeg: zod.boolean(),
        isAvailable: zod.boolean(),
        isBestseller: zod.boolean(),
        tags: zod.array(zod.string()),
        suggestedWith: zod.array(zod.string()),
      }),
    )
    .optional(),
  createdAt: zod.string(),
});

/**
 * @summary Delete restaurant (admin)
 */
export const DeleteRestaurantParams = zod.object({
  id: zod.coerce.string(),
});

/**
 * @summary List all orders (admin)
 */
export const ListAllOrdersQueryParams = zod.object({
  status: zod.coerce.string().optional(),
  page: zod.coerce.number().optional(),
});

export const listAllOrdersResponseItemsItemRatingMax = 5;

export const listAllOrdersResponseRestaurantRatingMax = 5;

export const ListAllOrdersResponseItem = zod.object({
  id: zod.string(),
  userId: zod.string(),
  userName: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  items: zod.array(
    zod.object({
      menuItemId: zod.string(),
      name: zod.string(),
      price: zod.number(),
      quantity: zod.number(),
      image: zod.string().nullish(),
      rating: zod
        .number()
        .min(1)
        .max(listAllOrdersResponseItemsItemRatingMax)
        .nullish(),
      review: zod.string().nullish(),
    }),
  ),
  status: zod.string(),
  paymentMethod: zod.string(),
  paymentStatus: zod.string(),
  subtotal: zod.number(),
  deliveryFee: zod.number(),
  discount: zod.number(),
  total: zod.number(),
  couponCode: zod.string().nullish(),
  coinsUsed: zod.number(),
  coinsEarned: zod.number(),
  deliveryAddress: zod.string(),
  estimatedDeliveryTime: zod.string().nullish(),
  trackingSteps: zod.array(
    zod.object({
      step: zod.string(),
      label: zod.string(),
      completedAt: zod.string().nullish(),
      isCompleted: zod.boolean(),
    }),
  ),
  createdAt: zod.string(),
  updatedAt: zod.string(),
  restaurantRating: zod
    .number()
    .min(1)
    .max(listAllOrdersResponseRestaurantRatingMax)
    .nullish(),
  restaurantReview: zod.string().nullish(),
});
export const ListAllOrdersResponse = zod.array(ListAllOrdersResponseItem);

/**
 * @summary Get revenue stats (admin)
 */
export const GetRevenueResponse = zod.object({
  totalRevenue: zod.number(),
  todayRevenue: zod.number(),
  weekRevenue: zod.number(),
  monthRevenue: zod.number(),
  totalOrders: zod.number(),
  completedOrders: zod.number(),
  cancelledOrders: zod.number(),
  averageOrderValue: zod.number(),
  topRestaurants: zod.array(
    zod.object({
      restaurantId: zod.string(),
      restaurantName: zod.string(),
      revenue: zod.number(),
      orders: zod.number(),
    }),
  ),
  revenueByDay: zod.array(
    zod.object({
      date: zod.string(),
      revenue: zod.number(),
      orders: zod.number(),
    }),
  ),
});

/**
 * @summary Get food suggestions based on cart
 */
export const GetSuggestionsQueryParams = zod.object({
  itemIds: zod.coerce.string().optional(),
});

export const GetSuggestionsResponseItem = zod.object({
  id: zod.string(),
  restaurantId: zod.string(),
  restaurantName: zod.string(),
  name: zod.string(),
  description: zod.string().nullish(),
  price: zod.number(),
  category: zod.string(),
  image: zod.string().nullish(),
  rating: zod.number(),
  totalRatings: zod.number(),
  isVeg: zod.boolean(),
  isAvailable: zod.boolean(),
  isBestseller: zod.boolean(),
  tags: zod.array(zod.string()),
  suggestedWith: zod.array(zod.string()),
});
export const GetSuggestionsResponse = zod.array(GetSuggestionsResponseItem);

/**
 * @summary Get admin dashboard stats
 */
export const GetAdminStatsResponse = zod.object({
  totalUsers: zod.number(),
  totalRestaurants: zod.number(),
  totalOrders: zod.number(),
  pendingOrders: zod.number(),
  activeOrders: zod.number(),
  totalRevenue: zod.number(),
  todayOrders: zod.number(),
  todayRevenue: zod.number(),
});
