...
import {
  ...
  useGetRestaurantMenu, useCreateMenuItem, useUpdateMenuItem, useDeleteMenuItem,
  getListRestaurantsQueryKey, getGetRestaurantMenuQueryKey,
} from "@workspace/api-client-react";
import { useAuth } from "@/lib/auth";
import { useToast } from "@/hooks/use-toast";
import { useQueryClient } from "@tanstack/react-query";
import {
  ChevronLeft, Plus, Star, Edit2, Trash2,
  Utensils, CheckCircle2, XCircle, AlertTriangle, Leaf, Search, Store
} from "lucide-react";
...
function MenuPanel({ restaurant, onClose }) {
  ...
  const { data: menuItems, isLoading } = useGetRestaurantMenu(
    restaurant.id, undefined,
    { query: { queryKey: getGetRestaurantMenuQueryKey(restaurant.id) } }
  );
    ...
    <DialogContent className="max-w-2xl max-h-[88vh] overflow-y-auto">
      ...
      <div className="space-y-4">
        ...
        {isLoading ? (
          ...
          <p className="text-center py-6 text-sm text-muted-foreground">No items match your search</p>
        ) : (
                  ...
                  {(items as any[]).map(item => (
                    ...
                    >
                      {item.image ? (
                        ...
                        <div className={`w-11 h-11 rounded-lg flex items-center justify-center shrink-0 text-xl ${item.isVeg ? "bg-green-50 dark:bg-green-900/20" : "bg-orange-50 dark:bg-orange-900/20"}`}>
                          {item.isVeg ? "🥗" : "🍗"}
                        </div>
                      )}
...
export default function AdminRestaurants() {
  ...
  if (user?.role !== "admin") return (
    <div className="container mx-auto px-4 py-20 text-center text-muted-foreground">Access Denied</div>
  );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(resto => (
            <Card key={resto.id} className={`border-border/50 overflow-hidden hover:shadow-lg transition-all group ${!resto.isOpen ? "opacity-70" : ""}`}>
              <div className="relative h-32 bg-gradient-to-br from-primary/10 to-secondary/10 overflow-hidden">
                {resto.image ? (
                  <img src={resto.image} alt={resto.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
                ...
                <div className="absolute bottom-2.5 left-3 right-3 flex items-end justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      onClick={() => handleToggleOpen(resto)}
                      title={resto.isOpen ? "Click to close" : "Click to open"}
                      className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-lg backdrop-blur-sm transition-all ${
                        resto.isOpen ? "bg-green-500/80 text-white hover:bg-red-500/80" : "bg-red-500/80 text-white hover:bg-green-500/80"
                      }`}
                    >
              ...
              <CardContent className="p-3 space-y-2.5">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="flex-1 h-8 text-xs border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground"
                    onClick={() => setMenuPanel(resto)}
                  >
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 w-8 p-0 border-border/50 hover:border-primary/40"
                    onClick={() => handleOpen(resto)}
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-8 w-8 p-0 border-destructive/30 text-destructive hover:bg-destructive hover:text-destructive-foreground hover:border-destructive"
                    onClick={() => setDeleteConfirm(resto)}
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-3 py-1">
            ...
            ].map(({ label, field, placeholder, num, span }) => (
