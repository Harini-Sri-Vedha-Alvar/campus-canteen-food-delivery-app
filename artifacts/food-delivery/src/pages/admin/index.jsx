...
export default function AdminDashboard() {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {isLoading ? ([1, 2, 3, 4].map(i => <Skeleton key={i} className="h-28 rounded-xl"/>)) : (<>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {[
            ...
            { title: "Manage Menu", desc: "Edit menu items and prices", href: "/admin/menu", icon: BarChart3, color: "text-orange-600" },
            { title: "Revenue", desc: "Track revenue and analytics", href: "/admin/revenue", icon: TrendingUp, color: "text-primary" },
      ...
      {stats?.recentOrders?.length > 0 && (<Card className="border-border/50">
          ...
          <CardContent>
            <div className="space-y-3">
              {stats.recentOrders.slice(0, 5).map((order) => (<div key={order.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-xs font-medium capitalize ${order.status === "delivered" ? "text-green-600" :
                    order.status === "cancelled" ? "text-red-600" :
                        "text-amber-600"}`}>{order.status.replace("_", " ")}</span>
                  </div>
                </div>))}
            </div>
          </CardContent>
