...
export default function Home() {
    ...
    const { data: restaurants, isLoading, error } = useListRestaurants(Object.keys(params).length > 0 ? params : undefined, { query: { queryKey: getListRestaurantsQueryKey(Object.keys(params).length > 0 ? params : undefined) } });
    const featured = Array.isArray(restaurants) ? restaurants.filter(r => r.featured) : [];
    const all = Array.isArray(restaurants) ? restaurants : [];
    return (<div className="w-full">
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex max-w-md mx-auto gap-2">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search restaurants or dishes..." className="pl-10 h-12 text-base" value={search} onChange={e => setSearch(e.target.value)} data-testid="input-search"/>
            </div>
      ...
      <section className="container mx-auto px-4 py-6">
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {CUISINES.map(c => (<button key={c} onClick={() => setCuisine(c === "All" ? "" : c)} className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all border ${(cuisine === c || (c === "All" && !cuisine))
                ? "bg-primary text-primary-foreground border-primary"
                : "bg-card border-border text-foreground hover:border-primary/50"}`} data-testid={`filter-cuisine-${c}`}>
              {c}
            </button>))}
        </div>
      </section>

      {featured.length > 0 && !search && !cuisine && (<section className="container mx-auto px-4 py-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {featured.slice(0, 3).map(r => (<RestaurantCard key={r.id} restaurant={r}/>))}
          </div>
        </section>)}
      ...
      <section className="container mx-auto px-4 py-6 pb-12">
        <div className="flex items-center gap-2 mb-4">
          ...
          {!isLoading && (<Badge variant="outline" className="text-muted-foreground">{all.length} places</Badge>)}
        </div>
        {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map(i => (<Skeleton key={i} className="h-64 rounded-xl"/>))}
          </div>) : all.length === 0 ? (<div className="text-center py-16 text-muted-foreground">
      ...
      <section className="bg-primary/5 border-y border-primary/10 py-10">
        ...
        ].map(coupon => (<div key={coupon.code} className="bg-card border border-dashed border-primary/40 rounded-xl p-4">
...
function RestaurantCard({ restaurant }) {
    return (<Link href={`/restaurant/${restaurant.id}`}>
      <Card className="overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer group border-border/50 hover:border-primary/30" data-testid={`card-restaurant-${restaurant.id}`}>
        <div className="relative h-44 overflow-hidden bg-muted">
          {restaurant.image ? (<img src={restaurant.image} alt={restaurant.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/>) : (<div className="w-full h-full flex items-center justify-center bg-primary/10">
              <span className="text-4xl">{restaurant.cuisine?.[0]}</span>
            </div>)}
          {!restaurant.isOpen && (<div className="absolute inset-0 bg-background/70 flex items-center justify-center">
              <Badge variant="outline" className="text-base font-semibold bg-background">Closed</Badge>
            </div>)}
          {restaurant.featured && (<Badge className="absolute top-2 left-2 bg-primary text-primary-foreground text-xs">
              Featured
            </Badge>)}
        </div>
        <CardContent className="p-4">
          ...
          {restaurant.tags?.length > 0 && (<div className="flex flex-wrap gap-1 mt-3">
              {restaurant.tags.slice(0, 3).map((tag) => (<Badge key={tag} variant="secondary" className="text-xs px-2 py-0 capitalize">{tag}</Badge>))}
            </div>)}
        </CardContent>
      </Card>
    </Link>);
