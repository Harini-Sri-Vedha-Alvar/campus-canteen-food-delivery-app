...
import {
  useGetCart, useUpdateCartItem, useRemoveCartItem, useClearCart, useCreateOrder,
  useValidateCoupon, useGetUserCoins, useGetSuggestions, useAddToCart,
...
import {
  ShoppingCart, Plus, Minus, Trash2, Tag, Coins, CreditCard, Banknote,
  ChevronRight, MapPin, Check, X, Smartphone, Scan, AlertCircle, Lock
} from "lucide-react";
...
function UPIQRCode({ upiId }) {
  return (
    <div className="flex flex-col items-center gap-3 p-4 bg-gradient-to-br from-primary/5 to-secondary/5 rounded-xl border border-primary/20">
      ...
      <div className="flex gap-2">
        {["gpay", "phonepe", "paytm", "bhim"].map(app => (
          <div key={app} className="h-10 w-10 rounded-xl bg-white shadow border border-border/50 flex items-center justify-center cursor-pointer hover:shadow-md transition-shadow">
...
function CardPaymentForm({ onCardChange }) {
  ...
  const update = (field, val) => {
    ...
    if (field === "number") setCardType(detectCardType(val));
  };
...
export default function Cart() {
  ...
  const { data: suggestions } = useGetSuggestions(
    ...
    { query: { enabled: !!cartItemIds, queryKey: getGetSuggestionsQueryKey(cartItemIds ? { itemIds: cartItemIds } : undefined) } }
  );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-5">
              ...
              {cart.items.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-4 hover:bg-muted/20 transition-colors">
                  {item.image ? (
                    ...
                    <div className="w-16 h-16 rounded-xl bg-primary/10 shrink-0 flex items-center justify-center text-2xl">🍽️</div>
                  )}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="flex items-center gap-1 bg-primary rounded-lg overflow-hidden">
                      <button
                        className="h-8 w-8 flex items-center justify-center text-primary-foreground hover:bg-black/10 transition-colors"
            ...
            <div>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {suggestions.slice(0, 5).map(item => (
                  <Card key={item.id} className="shrink-0 w-36 border-border/50 hover:border-primary/30 transition-colors cursor-pointer group">
                    ...
                    <CardContent className="p-2">
                      ...
                      <Button size="sm" variant="outline" className="w-full mt-1 h-7 text-xs border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground"
                        onClick={async () => {
                          try {
            ...
            <CardContent className="space-y-3">
              <div className={`relative rounded-xl border-2 transition-all ${addressConfirmed ? "border-green-500 bg-green-50 dark:bg-green-900/10" : "border-border"}`}>
                <textarea
                  ...
                  className="w-full bg-transparent p-3 text-sm resize-none focus:outline-none min-h-[72px] pr-10"
                  value={deliveryAddress}
                  onChange={e => { setDeliveryAddress(e.target.value); setAddressConfirmed(false); }}
                  rows={3}
                />
                {addressConfirmed && (
              ...
              {deliveryAddress.trim() && !addressConfirmed && (
                <Button
                  variant="outline"
          ...
          <Card className="border-border/50">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 gap-2">
                {PAYMENT_METHODS.map(method => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all text-left ${paymentMethod === method.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/30 hover:bg-muted/30"}`}
                  >
              ...
              {paymentMethod === "cod" && (
                <div className="mt-2 flex items-start gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <CardContent className="space-y-3">
              {!appliedCoupon ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SAMPLE_COUPONS.map(c => (
                      <button
                        ...
                        className="text-xs font-mono px-2.5 py-1 rounded-lg border border-dashed border-primary/40 text-primary hover:bg-primary/10 transition-colors"
                      >
                        {c}
                      </button>
                    ))}
                ...
                <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
                  ...
                  <button
                    onClick={() => { setAppliedCoupon(null); setCouponCode(""); toast({ title: "Coupon removed" }); }}
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${useCoins ? "bg-amber-500" : "bg-muted"}`}
                  >
          ...
          {!addressConfirmed && deliveryAddress.trim() === "" && (
            <div className="flex items-start gap-2 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3 border border-amber-200 dark:border-amber-800">
      ...
      <Dialog open={showAddressModal} onOpenChange={setShowAddressModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <textarea
              className="w-full rounded-xl border-2 border-border p-3 text-sm focus:outline-none focus:border-primary min-h-[80px] bg-background resize-none"
              placeholder="Enter complete address with flat/house no, street, area, city..."
              value={deliveryAddress}
