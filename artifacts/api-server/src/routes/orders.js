...
router.post("/orders/:id/rate", requireAuth, async (req, res) => {
    const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const { restaurantRating, restaurantReview, itemRatings } = req.body;
    ...
    if (restaurantRating) {
        ...
        order.restaurantReview = restaurantReview || null;
        
        // Update restaurant rating
        const restaurant = await Restaurant.findById(order.restaurantId);
        if (restaurant) {
            const allRatings = await Order.find({ restaurantId: order.restaurantId, restaurantRating: { $ne: null } });
            const totalRating = allRatings.reduce((sum, o) => sum + o.restaurantRating, 0) + restaurantRating;
            restaurant.rating = totalRating / (allRatings.length + 1);
            restaurant.totalRatings = allRatings.length + 1;
    ...
    if (itemRatings && Array.isArray(itemRatings)) {
        for (const itemRating of itemRatings) {
            const item = order.items.find(i => i.menuItemId === itemRating.menuItemId);
            if (item) {
                item.rating = itemRating.rating;
                item.review = itemRating.review || null;
            }
        }
    }
