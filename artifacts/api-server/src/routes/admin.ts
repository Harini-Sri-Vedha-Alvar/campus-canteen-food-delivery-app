...
router.post("/admin/restaurants", requireAuth, requireAdmin, async (req: Request, res: Response): Promise<void> => {
...
router.put("/admin/restaurants/:id", requireAuth, requireAdmin, async (req: Request, res: Response): Promise<void> => {
...
router.delete("/admin/restaurants/:id", requireAuth, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
...
router.get("/admin/revenue", requireAuth, requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  const now = new Date();
  ...
  const restaurantRevMap: Record<string, { restaurantId: string, restaurantName: string, revenue: number, orders: number }> = {};
  ...
  const revenueByDayMap: Record<string, { date: string, revenue: number, orders: number }> = {};
  for (let i = 6; i >= 0; i--) {
...
router.get("/admin/stats", requireAuth, requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  const now = new Date();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  ...
  const todayRevenue = todayOrders.filter((o: any) => o.status === "delivered").reduce((s: number, o: any) => s + o.total, 0);
  res.json({
    totalUsers,
    totalRestaurants,
    totalOrders,
