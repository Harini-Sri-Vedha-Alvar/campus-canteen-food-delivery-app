...
function computeCartTotals(cart: any, deliveryFee: number) {
...
function formatCart(cart: any, deliveryFee: number) {
...
router.get("/cart", requireAuth, async (req: Request, res: Response): Promise<void> => {
...
router.post("/cart", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const existingIdx = cart.items.findIndex((i: any) => i.menuItemId === menuItemId);
  if (existingIdx >= 0) {
...
router.patch("/cart/:itemId", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const idx = cart.items.findIndex((i: any) => i._id.toString() === itemId);
  if (idx < 0) {
...
router.delete("/cart/:itemId", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const userId = (req as any).user.userId;
  const { itemId } = req.params;
  const cart = await Cart.findOne({ userId });
  if (!cart) {
...
router.delete("/cart", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const userId = (req as any).user.userId;
